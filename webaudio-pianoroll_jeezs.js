// function playNoteAudio(audioContext, frequency, duration, startTime) {
...
//
//     // Schedule the note to start and stop at the correct time
...
customElements.define("webaudio-pianoroll", class Pianoroll extends HTMLElement {
    constructor() {
        super();
        this.noteIdCounter = 0; // Initialise le compteur d'ID
        this.sequence = [];
    }

    setTempo(newTempo) {
        this.tempo = newTempo;
        this.updateTimer(); // Appeler cette méthode pour mettre à jour les calculs basés sur le nouveau tempo
        console.log(`Tempo mis à jour : ${newTempo} BPM`);
    }

    // Méthode updateTimer existante pour recalculer les temps de tick
    updateTimer() {
...
<div id="wac-menu">Delete!</div>
        ...
        // this.play = function (actx, playcallback, positionCallback, tick) {
        //     function Interval() {
        ...
        //         this.redrawMarker();
        //
        //         // Call the position callback with the current cursor position
        //         if (typeof positionCallback === 'function') {
        //             positionCallback(this.cursor, this.cursor * (60 / this.tempo / this.timebase));
        //         }
        //
        ...
        this.play = function (actx, playcallback, positionCallback, tick) {
            const that = this; // Stockez la référence à l'objet actuel

            function Interval() {
                const current = that.actx.currentTime;
                while (that.timestack.length > 1 && current >= that.timestack[1][0]) {
                    that.timestack.shift();
                }
                that.cursor = that.timestack[0][1] + (current - that.timestack[0][0]) / that.timestack[0][2];
                that.redrawMarker();

                // Appel de la fonction de callback pour la position
                if (typeof positionCallback === 'function') {
                    positionCallback(that.cursor, that.cursor * (60 / that.tempo / that.timebase));
                }

                while (current + that.preload >= that.time1) {
                    that.time0 = that.time1;
                    that.tick0 = that.tick1;
                    let e = that.sequence[that.index1];
                    if (!e || e.t >= that.markend) {
                        that.timestack.push([that.time1, that.markstart, that.tick2time]);
                        const p = that.findNextEv(that.markstart);
                        that.time1 += p.dt * that.tick2time;
                        that.index1 = p.i;
                    } else {
                        that.tick1 = e.t;
                        that.timestack.push([that.time1, e.t, that.tick2time]);
                        let gmax = Math.min(e.t + e.g, that.markend) - e.t;
                        if (that.editmode == "gridmono" || that.editmode == "gridpoly")
                            gmax *= that.gridnoteratio;
                        const cbev = { t: that.time1, g: that.time1 + gmax * that.tick2time, n: e.n };
                        if (that.playcallback)
                            that.playcallback(cbev);
                        e = that.sequence[++that.index1];
                        if (!e || e.t >= that.markend) {
                            that.time1 += (that.markend - that.tick1) * that.tick2time;
                            const p = that.findNextEv(that.markstart);
                            that.timestack.push([that.time1, that.markstart, that.tick2time]);
                            that.time1 += p.dt * that.tick2time;
                            that.index1 = p.i;
                        } else
                            that.time1 += (e.t - that.tick1) * that.tick2time;
                    }
                }
            }
            ...
            function updateMarker() {
                that.redrawMarker();
                requestAnimationFrame(updateMarker); // Continuez à appeler updateMarker
            }
            ...
            this.timer = setInterval(Interval.bind(this), 25); // Garder setInterval pour le traitement des événements
            requestAnimationFrame(updateMarker); // Démarrer le rafraîchissement du marqueur
        };
        this.stop = function () {
            if (this.timer)
            ...
            for (parse.i = 0; parse.i < parse.s.length;) {
                ...
                if (n >= 0) {
                    ...
                    if (tie && evlast && evlast.n == n) {
                        ...
                        this.sequence.push(evlast = {  id: this.noteIdCounter++, t: t, n: n, g: l, f: 0});
                }
                t += l;
            }
            this.redraw();
        ...
        this.addNote = function (t, n, g, v, f) {
            if (t >= 0 && n >= 0 && n < 128) {
                const id = this.noteIdCounter++; // Génère et incrémente l'ID unique
                const ev = { id: id, t: t, n: n, g: g, v: v, f: f };
                this.sequence.push(ev);
                console.log('note added ' + n)
                console.log('note add also ' + ev.id)
                this.sortSequence();
                this.redraw();
                return ev;
            }
        ...
        this.delNote = function (idx) {
            alert('message from : this.delNote');
            const deletedNote = this.sequence[idx]; // Capture the note being deleted

            // Remove the note from the sequence
            this.sequence.splice(idx, 1);

            // Dispatch the noteDeleted event with the deleted note's details
            ...
            // Redraw the piano roll to reflect the changes
            this.redraw();
        };
        ...
        this.delSelectedNote = function () {
            const l = this.sequence.length;
            const deletedNotes = []; // Array to store the deleted notes

            for (let i = l - 1; i >= 0; --i) {
                const ev = this.sequence[i];
                if (ev.f) {
                    deletedNotes.push(ev); // Add the note to the deletedNotes array
                    this.sequence.splice(i, 1);
                }
            }

            // Log the details of the deleted notes
            console.log('Deleted Notes:', deletedNotes);
            //	•	t: 32: The note starts at tick 32.
            // 	•	n: 66: The note is F#4.
            // 	•	g: 6: The note has a duration of 6 ticks.
            // 	•	f: 1: The note was selected (likely for deletion).
        };
        ...
        this.moveSelectedNote = function (dt, dn) {
            ...
            for (let i = 0; i < l; ++i) {
                ...
                if (ev.f) {
                    ev.t = (((ev.ot + dt) / this.snap + .5) | 0) * this.snap;
                    ev.n = ev.on + dn;
                    console.log('id found: ' + ev.id + ' changing note position: ' + ': ' + ev.g + ': ' + ev.t + ': ' + ev.n + ': ' + ev.f);
                }
            }
        };
        ...
        this.editDragDown = function (pos) {
            ...
            if (ht.m == "N") {
                ...
                /// Jeezs add on
                console.log('unknow case 1: '+ev.id)
                /// end add on
                this.redraw();
            } else if (ht.m == "n") {
                ev = this.sequence[ht.i];
                ...
                const id = this.noteIdCounter++; // Génère et incrémente l'ID unique
                // const ev = { id: id, t: t, n: n, g: g, v: v, f: f };
                let newNote = { id: id,t: t, n: ht.n | 0, g: 1, f: 1};
                console.log('New note added:', newNote); // Ajoutez ce log
                console.log('Sequence content:', JSON.stringify(this.sequence, null, 2)); // Affichez le contenu de this.sequence dans la console
                // end add on
                ...
                /// Jeezs add on
                console.log('unknow 5, raw  note : '+ht.n)
                /// end add on
                this.redraw();
            }
        };
        ...
        this.pointermove = function (ev) {
            ...
            switch (this.dragging.o) {
                ...
                case "P":
                    ...
                    // Log the current position of the playhead in ticks and time
                    console.log(`Playhead position - Tick: ${this.cursor}, Time: ${(this.cursor * (60 / this.tempo / this.timebase)).toFixed(3)} seconds`);
                    break;
            }
...
class Pianoroll extends HTMLElement {
    constructor() {
        super();
        // Initialize your Pianoroll component here
    }
...
// Function to play audio for a note
// function playNoteAudio(audioContext, frequency, duration, startTime) {
//     console.log('play!!');
//     if (audioContext.state === 'suspended') {
//         console.log('suspended play!!');
//         requestAnimationFrame(() => playNoteAudio(audioContext, frequency, duration, startTime));
//     } else {
...
function playNoteAudio(audioContext, frequency, duration, startTime) {
    if (audioContext.state === 'suspended') {
        audioContext.resume().then(() => {
            playNoteAudio(audioContext, frequency, duration, audioContext.currentTime + 0.1); // Ajustez ici
        });
        return;
    }
    ...
    const actualStartTime = audioContext.currentTime + 0.1; // Utilisez le temps courant ajusté
...
function play() {
    initializeAudioContext();  // Initialize the AudioContext if it hasn't been already
    actx.resume().then(() => {
        let pianoRoll = document.querySelector('webaudio-pianoroll');
        startTime = actx.currentTime;  // Capture the start time of the playback

        // Play the sequence
        pianoRoll.play(actx, function(ev) {
            const frequency = 440 * Math.pow(2, (ev.n - 69) / 12);
            const duration = ev.g * (60 / pianoRoll.tempo / pianoRoll.timebase);
            const noteStartTime = startTime + (ev.t * (60 / pianoRoll.tempo / pianoRoll.timebase));

            // Schedule and play the note
            playNoteAudio(actx, frequency, duration, noteStartTime);
            console.log(`Note played: ${ev.n}, Start time: ${noteStartTime.toFixed(3)}, Duration: ${duration.toFixed(3)}`);
        }, function(cursor, timeInSeconds) {
            // Log the current position of the playhead during playback
            console.log(`Current playhead position - Tick: ${cursor}, Time: ${timeInSeconds.toFixed(3)} seconds`);
        });
    });
}
// function Play() {
//     actx.resume(); // Resume the audio context if it's in a suspended state
//     let pianoRoll = document.querySelector('webaudio-pianoroll');
//     startTime = actx.currentTime; // Capture the start time of the playback
//     // alert(JSON.stringify(pianoRoll.sequence)); // Utilisez JSON.stringify pour afficher le contenu de l'objet
//
//     // Play the sequence
//     pianoRoll.play(actx, function(ev) {
//         const frequency = 440 * Math.pow(2, (ev.n - 69) / 12);
...
function Stop() {
    let pianoRoll = document.querySelector('webaudio-pianoroll');
    pianoRoll.stop(); // Stop the playback
}
...
function AddNote() {
    actx.resume();
    let sequence = document.getElementById("pianoroll");
    sequence.addNote(
        0, // Track
        60, // Tick
        60, // Duration
...
// Changer le tempo à 33 BPM
    pianoRoll.setTempo(100);
